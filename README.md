...
- Socket과 Thread를 사용하여 구현한 멀티 클라이언트 채팅 프로그램입니다. 서버와 다수의 클라이언트가 네트워크 환경에서 통신하며 클라이언트의 메시지를 모든 사용자에게 전달하는 기능을 구현하였고
...
**후기**
- thread를 통해 병렬적으로 client socket를 연결하고 broadcast로 보냄으로써 파일 디스크립터를 사용하는 select방식보다 효율적이라고 생각했습니다.